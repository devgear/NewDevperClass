...
uses Windows, Messages, CommCtrl, ActiveX, OleDlg, SysUtils, Classes,
  Controls, Forms, Menus, Graphics, ComObj, OleCtnrDummyFormUnit;
...
type
  TModOleContainer = class;
  TOleForm = class;

  IVCLFrameForm = interface(IOleInPlaceFrame)
    ['{CD02E1C0-52DA-11D0-9EA6-0020AF3D82DA}']
    procedure AddContainer(Instance: TModOleContainer);
    procedure RemoveContainer(Instance: TModOleContainer);
  ...
  TObjectMoveEvent = procedure(OleContainer: TModOleContainer;
    const Bounds: TRect) of object;

  TModOleContainer = class(TCustomControl, IUnknown, IOleClientSite,
  ...
  published
    ...
    property AutoActivate: TAutoActivate read FAutoActivate write FAutoActivate default aaManual;
    ...
    property Ctl3D;
    property AutoDeactivate: boolean read FAutoDeactivate write FAutoDeactivate default false;
    property DragCursor;
    property DragMode;
    property Enabled;
...
procedure register;
begin
  RegisterComponents('Samples', [TModOleContainer]);
end;
...
type
  ...
  private
    FContainer: TModOleContainer;
    FOleLink: IOleLink;
  public
    constructor Create(Container: TModOleContainer);
...
constructor TOleUILinkInfo.Create(Container: TModOleContainer);
begin
  inherited Create;
...
begin
  ...
  UTNCharLen := MultiByteToWideChar(0, 0, PAnsichar(UserTypeName),
      Length(UserTypeName), nil, 0) + 1;
  SOCCharLen := MultiByteToWideChar(0, 0, PAnsichar(SourceOfCopy),
  ...
  begin
    ...
    begin
      ...
      MultiByteToWideChar(0, 0, PAnsichar(UserTypeName), Length(UserTypeName),
        P, UTNCharLen);
      P[UTNCharLen-1] := #0;

      dwSrcOfCopy := dwFullUserTypeName + SOCCharLen * SizeOf(WideChar);
      P := PWideChar(Integer(Descriptor) + dwSrcOfCopy);
      MultiByteToWideChar(0, 0, PAnsichar(SourceOfCopy), Length(SourceOfCopy),
        P, SOCCharLen);
      P[SOCCharLen-1] := #0;
    end;
    GlobalUnlock(Result);
...
{ TModOleContainer.IOleUIObjInfo - helper interface for Object Properties dialog }

function TModOleContainer.GetObjectInfo(dwObject: Longint;
  var dwObjSize: Longint; var lpszLabel: PChar;
  var lpszType: PChar; var lpszShortType: PChar;
...
function TModOleContainer.GetConvertInfo(dwObject: Longint; var ClassID: TCLSID;
  var wFormat: Word; var ConvertDefaultClassID: TCLSID;
...
function TModOleContainer.ConvertObject(dwObject: Longint;
  const clsidNew: TCLSID): HResult;
begin
  Result := E_NOTIMPL;
end;

function TModOleContainer.GetViewInfo(dwObject: Longint; var hMetaPict: HGlobal;
...
function TModOleContainer.SetViewInfo(dwObject: Longint; hMetaPict: HGlobal;
  dvAspect: Longint; nCurrentScale: Integer;
...
{ TModOleContainer.IOleClientSite }

function TModOleContainer.GetMoniker(dwAssign: Longint; dwWhichMoniker: Longint;
...
function TModOleContainer.GetContainer(out container: IOleContainer): HResult;
begin
  container := nil;
  Result := E_NOINTERFACE;
end;

function TModOleContainer.ShowObject: HResult;
begin
  Result := S_OK;
end;

function TModOleContainer.OnShowWindow(fShow: BOOL): HResult;
begin
...
function TModOleContainer.RequestNewObjectLayout: HResult;
begin
  Result := E_NOTIMPL;
end;

{ TModOleContainer.IOleInPlaceSite }

function TModOleContainer.GetWindow(out wnd: HWnd): HResult;
begin
  if FDocObj then
    wnd := Handle
  else
...
function TModOleContainer.ContextSensitiveHelp(fEnterMode: BOOL): HResult;
begin
  Result := S_OK;
end;

function TModOleContainer.CanInPlaceActivate: HResult;
begin
  Result := S_FALSE;
...
function TModOleContainer.OnInPlaceActivate: HResult;
begin
...
function TModOleContainer.OnUIActivate: HResult;
begin
  SetUIActive(True);
  Result := S_OK;
end;

function TModOleContainer.GetWindowContext(out frame: IOleInPlaceFrame;
  out doc: IOleInPlaceUIWindow; out rcPosRect: TRect;
  out rcClipRect: TRect; out frameInfo: TOleInPlaceFrameInfo): HResult;
var
  Origin: TPoint;
...
function TModOleContainer.Scroll(scrollExtent: TPoint): HResult;
begin
  Result := E_NOTIMPL;
end;

function TModOleContainer.OnUIDeactivate(fUndoable: BOOL): HResult;
begin
...
function TModOleContainer.OnInPlaceDeactivate: HResult;
begin
...
function TModOleContainer.DiscardUndoState: HResult;
begin
  Result := E_NOTIMPL;
end;

procedure TModOleContainer.Deactivate;
begin
  FDummyForm.ActiveOleControl.Perform(CM_UIDEACTIVATE, 0, 0);
end;

function TModOleContainer.DeactivateAndUndo: HResult;
begin
  FOleInPlaceObject.UIDeactivate;
  Result := S_OK;
end;

function TModOleContainer.OnPosRectChange(const rcPosRect: TRect): HResult;
begin
  try
...
{ TModOleContainer.IAdviseSink }

procedure TModOleContainer.OnDataChange(const formatetc: TFormatEtc;
  const stgmed: TStgMedium);
begin
  Changed;
end;

procedure TModOleContainer.OnViewChange(dwAspect: Longint; lindex: Longint);
begin
  if dwAspect = FDrawAspect then UpdateView;
end;

procedure TModOleContainer.OnRename(const mk: IMoniker);
begin
end;

procedure TModOleContainer.OnSave;
begin
end;

procedure TModOleContainer.OnClose;
begin
end;

{ TModOleContainer.IOleDocumentSite }

function TModOleContainer.ActivateMe(View: IOleDocumentView): HRESULT;
var
  Doc: IOleDocument;
begin
  Result := E_FAIL;
...
constructor TModOleContainer.Create(AOwner: TComponent);
const
...
begin
  ...
  FAutoActivate := aaManual;
  FAutoVerbMenu := True;
  FBorderStyle := bsSingle;
  FCopyOnSave := True;
  FDrawAspect := DVASPECT_CONTENT;

  FDummyForm := TOleCtnrDummyForm.Create(Self);
  FDummyForm.Width := 0;
  FDummyForm.Height := 0;
  FDummyForm.Parent := nil;
end;

destructor TModOleContainer.Destroy;
begin
  DestroyObject;
  FDummyForm.Free;
  inherited Destroy;
...
function TModOleContainer._AddRef: Integer;
begin
  Inc(FRefCount);
  Result := FRefCount;
end;

procedure TModOleContainer.AdjustBounds;
var
  Size: TPoint;
  Extra: Integer;
begin
...
function TModOleContainer.ChangeIconDialog: Boolean;
var
...
procedure TModOleContainer.CreateLinkToFile(const FileName: string;
  Iconic: Boolean);
var
...
procedure TModOleContainer.CreateObject(const OleClassName: string;
  Iconic: Boolean);
var
  CreateInfo: TCreateInfo;
begin
...
procedure TModOleContainer.CreateObjectFromFile(const FileName: string;
  Iconic: Boolean);
var
  CreateInfo: TCreateInfo;
begin
...
procedure TModOleContainer.CreateObjectFromInfo(const CreateInfo: TCreateInfo);
begin
  DestroyObject;
  try
    CreateStorage;
...
procedure TModOleContainer.CreateParams(var Params: TCreateParams);
begin
...
procedure TModOleContainer.CreateStorage;
begin
...
procedure TModOleContainer.DblClick;
begin
...
procedure TModOleContainer.DefineProperties(Filer: TFiler);
begin
...
procedure TModOleContainer.DesignModified;
var
  Form: TCustomForm;
begin
...
procedure TModOleContainer.DestroyAccelTable;
begin
  if FAccelTable <> 0 then
  begin
...
procedure TModOleContainer.DestroyObject;
var
  DataObject: IDataObject;
begin
  if FOleObject <> nil then
...
procedure TModOleContainer.DestroyVerbs;
begin
  FPopupVerbMenu.Free;
  FPopupVerbMenu := nil;
  FObjectVerbs.Free;
...
procedure TModOleContainer.DoEnter;
begin
  if FAutoActivate = aaGetFocus then DoVerb(ovShow);
  inherited;
end;

procedure TModOleContainer.DoVerb(Verb: Integer);
var
  H: THandle;
  R: TRect;
begin
...
function TModOleContainer.GetBorderWidth: Integer;
begin
...
function TModOleContainer.GetCanPaste: Boolean;
var
  DataObject: IDataObject;
begin
...
function TModOleContainer.GetIconic: Boolean;
begin
  Result := FDrawAspect = DVASPECT_ICON;
end;

function TModOleContainer.GetIconMetaPict: HGlobal;
var
  DataObject: IDataObject;
...
function TModOleContainer.GetLinked: Boolean;
var
  OleLink: IOleLink;
begin
  CheckObject;
...
function TModOleContainer.GetObjectDataSize: Integer;
var
  DataHandle: HGlobal;
begin
...
function TModOleContainer.GetObjectVerbs: TStrings;
begin
  if FObjectVerbs = nil then UpdateVerbs;
  Result := FObjectVerbs;
end;

function TModOleContainer.GetOleClassName: string;
var
  ClassID: TCLSID;
begin
  CheckObject;
...
function TModOleContainer.GetOleObject: Variant;
begin
  CheckObject;
  Result := Variant(FOleObject as IDispatch);
end;

function TModOleContainer.GetPopupMenu: TPopupMenu;
var
  I: Integer;
  Item: TMenuItem;
begin
...
function TModOleContainer.GetPrimaryVerb: Integer;
begin
  if FObjectVerbs = nil then UpdateVerbs;
...
function TModOleContainer.GetSourceDoc: string;
var
  OleLink: IOleLink;
begin
  CheckObject;
...
function TModOleContainer.GetState: TObjectState;
begin
  if FOleObject = nil then
    Result := osEmpty
...
procedure TModOleContainer.InitObject;
...
var
  ...
  DocForm: TCustomForm;
begin
  DocForm := FDummyForm; // GetParentForm(Self, false);
  if DocForm = nil then
...
function TModOleContainer.InsertObjectDialog: Boolean;
var
...
procedure TModOleContainer.KeyDown(var Key: Word; Shift: TShiftState);
begin
...
procedure TModOleContainer.LoadFromFile(const FileName: string);
var
  Stream: TStream;
begin
...
procedure TModOleContainer.LoadFromStream(Stream: TStream);
var
  DataHandle: HGlobal;
  Buffer: Pointer;
...
procedure TModOleContainer.MouseDown(Button: TMouseButton;
...
procedure TModOleContainer.Changed;
begin
...
procedure TModOleContainer.ObjectMoved(const ObjectRect: TRect);
var
  R: TRect;
  I: Integer;
begin
...
function TModOleContainer.ObjectPropertiesDialog: Boolean;
var
...
procedure TModOleContainer.Paint;
var
  W, H: Integer;
  S: TPoint;
  R, CR: TRect;
...
procedure TModOleContainer.Paste;
var
  DataObject: IDataObject;
  Descriptor: PObjectDescriptor;
  FormatEtc: TFormatEtc;
...
function TModOleContainer.PasteSpecialDialog: Boolean;
const
  PasteFormatCount = 2;
var
...
procedure TModOleContainer.PopupVerbMenuClick(Sender: TObject);
begin
  DoVerb((Sender as TMenuItem).Tag);
end;

function TModOleContainer.QueryInterface(const iid: TIID; out obj): HResult;
begin
  Pointer(obj) := nil;
  Result := E_NOINTERFACE;
...
function TModOleContainer._Release: Integer;
begin
  Dec(FRefCount);
  Result := FRefCount;
end;

procedure TModOleContainer.Run;
begin
  CheckObject;
  OleCheck(OleRun(FOleObject));
end;

function TModOleContainer.SaveObject: HResult;
var
  PersistStorage: IPersistStorage;
begin
  Result := S_OK;
...
procedure TModOleContainer.SaveAsDocument(const FileName: string);
var
...
procedure TModOleContainer.SaveToFile(const FileName: string);
var
  Stream: TStream;
begin
...
procedure TModOleContainer.SaveToStream(Stream: TStream);
var
...
procedure TModOleContainer.SetBorderStyle(Value: TBorderStyle);
begin
...
procedure TModOleContainer.SetDrawAspect(Iconic: Boolean;
  IconMetaPict: HGlobal);
var
  OleCache: IOleCache;
...
procedure TModOleContainer.SetFocused(Value: Boolean);
var
  R: TRect;
begin
...
procedure TModOleContainer.SetIconic(Value: Boolean);
begin
...
procedure TModOleContainer.SetSizeMode(Value: TSizeMode);
begin
...
procedure TModOleContainer.SetUIActive(Active: Boolean);
var
  Form: TCustomForm;
begin
  try
    FUIActive := Active;
    Form := FDummyForm; // GetParentForm(Self, false);
    if Form <> nil then
      if Active then
      begin
...
procedure TModOleContainer.SetViewAdviseSink(Enable: Boolean);
var
...
procedure TModOleContainer.UpdateObject;
begin
  if FOleObject <> nil then
  begin
...
procedure TModOleContainer.UpdateObjectRect;
var
  P: TPoint;
  R: TRect;
begin
...
procedure TModOleContainer.UpdateVerbs;
var
...
procedure TModOleContainer.UpdateView;
var
...
procedure TModOleContainer.CMCtl3DChanged(var Message: TMessage);
begin
...
procedure TModOleContainer.CMDocWindowActivate(var Message: TMessage);
begin
...
procedure TModOleContainer.CMUIDeactivate(var Message: TMessage);
begin
  OutputDebugString(PChar('>>>>> Deactivating.. '));
  if (FAutoDeactivate or (csDestroying in ComponentState)) and
     (FDummyForm.ActiveOleControl = Self) and (FOleInPlaceObject <> nil) then
//     (GetParentForm(Self, false).ActiveOleControl = Self) and (FOleInPlaceObject <> nil) then
    begin
      FOleInPlaceObject.UIDeactivate;
    end;
end;

procedure TModOleContainer.WMKillFocus(var Message: TWMSetFocus);
begin
  inherited;
  SetFocused(False);
end;

procedure TModOleContainer.WMSetFocus(var Message: TWMSetFocus);
var
  Window: HWnd;
begin
  inherited;
...
procedure TModOleContainer.WMSize(var Message: TWMSize);
begin
  inherited;
...
procedure TModOleContainer.WMWindowPosChanged(var Message: TWMWindowPosChanged);
var
  R: TRect;
begin
  R := BoundsRect;
...
begin
  for I := FContainers.Count - 1 downto 0 do
    TModOleContainer(FContainers[I]).DestroyObject;
end;
...
procedure TOleForm.AddContainer(Instance: TModOleContainer);
begin
  FContainers.Add(Instance);
end;

procedure TOleForm.RemoveContainer(Instance: TModOleContainer);
begin
